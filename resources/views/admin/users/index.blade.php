            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('admin.users.index') }}"
                        class="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($users as $user)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $user->department->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $user->university->name ?? 'N/A' }}</td>
                                            ...
                                            @can('toggleStatus', $user)
                                                <form action="{{ route('admin.users.toggle-status', $user) }}" method="POST" class="inline">
                                                    ...
                                                    <button type="submit" class="focus:outline-none">
                                                        @if($user->is_active)
                                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 hover:bg-green-200">Active</span>
                                                        @else
                                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 hover:bg-red-200">Inactive</span>
                                                        @endif
                                                    </button>
                                                </form>
                                            @endcan
