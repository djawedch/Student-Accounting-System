<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Scholarship Award') }} #{{ $award->id }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.scholarship-awards.update', $award) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="student_id" id="student_id"
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ old('student_id', $award->student_id) == $student->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="scholarship_id" id="scholarship_id"
                                ...
                                @foreach($scholarships as $scholarship)
                                    <option value="{{ $scholarship->id }}" {{ old('scholarship_id', $award->scholarship_id) == $scholarship->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="grant_date" id="grant_date"
                                value="{{ old('grant_date', $award->grant_date->format('Y-m-d')) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="end_date" id="end_date"
                                value="{{ old('end_date', $award->end_date ? $award->end_date->format('Y-m-d') : '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status" id="status"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                                <option value="awarded" {{ old('status', $award->status) == 'awarded' ? 'selected' : '' }}>Awarded</option>
                                <option value="paid" {{ old('status', $award->status) == 'paid' ? 'selected' : '' }}>Paid</option>
                                <option value="cancelled" {{ old('status', $award->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="paid_at" id="paid_at"
                                value="{{ old('paid_at', $award->paid_at ? $award->paid_at->format('Y-m-d') : '') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="reference" id="reference"
                                value="{{ old('reference', $award->reference) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
