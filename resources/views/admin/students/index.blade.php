            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-2xl font-semibold text-gray-900">All Students</h2>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Study System</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($students as $student)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->first_name }} {{ $student->last_name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->email }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->department->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->department->university->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->student->level ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ ucfirst($student->student->study_system ?? 'N/A') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ $student->student->academic_year ?? 'N/A' }}</td>
                                            ...
                                            @can('toggleStatus', $student)
                                                <form action="{{ route('admin.students.toggle-status', $student) }}" method="POST" class="inline">
                                                    ...
                                                    <button type="submit" class="focus:outline-none">
                                                        @if($student->is_active)
                                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 hover:bg-green-200 transition">Active</span>
                                                        @else
                                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 hover:bg-red-200 transition">Inactive</span>
                                                        @endif
                                                    </button>
                                                </form>
                                            @endcan
