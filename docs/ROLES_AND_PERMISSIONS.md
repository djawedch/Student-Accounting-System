...
- **super_admin** → full CRUD except delete (create, read, update, toggle status) on any admin user. Cannot delete users (only deactivate via toggleStatus). Can create/update users with any role lower than super_admin (i.e., university_admin, department_admin, staff_admin). Can view all admin users. Cannot manage themselves (update/toggle self blocked by policy). Can assign any university/department.
- **university_admin** → can list, view, create, update, and toggle status only for users who belong to the same university and have a strictly lower role rank (department_admin, staff_admin). Cannot manage themselves, cannot manage super_admin or other university_admins. When creating a user, role choices are limited to ranks lower than university_admin (department_admin, staff_admin). Student creation is not allowed here; students must be created via StudentController. University selection is fixed to their own university; department choices are limited to departments within their university.
- **department_admin** → can list, view, create, update, and toggle status only for users who belong to the same department and have a strictly lower role rank (staff_admin). Cannot manage themselves, cannot manage users of equal or higher rank (including other department_admins). When creating a user, role choices are limited to ranks lower than department_admin (staff_admin). Student creation is not allowed here; students must be created via StudentController. Department selection is fixed to their own department; university is auto assigned from that department.
- **staff_admin** → can list and view only users who belong to the same department and have a role of department_admin or staff_admin (i.e., equal or higher rank within the same department). No create, update, delete, or toggle status actions. Cannot manage themselves (viewing is allowed, but no modifications). Department scope is fixed to their own department.
...
- **super_admin** → full CRUD except delete (create, read, update, toggleStatus) on any student. Can assign student to any university and any department. Can toggle student active/inactive status. All student operations allowed without hierarchical restrictions.
