...
- **staff_admin** → can list and view only users who belong to the same department and have a role rank equal to or lower than staff_admin (i.e., staff_admin and student). No create, update, delete, or toggle status actions. Cannot manage themselves (viewing is allowed, but no modifications). Department scope is fixed to their own department.
