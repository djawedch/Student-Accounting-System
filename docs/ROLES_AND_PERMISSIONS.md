...
- **super_admin** → full CRUD except delete (create, read, update, toggle status) on any admin user. Cannot delete users (only deactivate via toggleStatus). Can create/update users with any role lower than super_admin (i.e., university_admin, department_admin, staff_admin, student — but student creation might be elsewhere). Can view all admin users. Cannot manage themselves (update/toggle self blocked by policy). Can assign any university/department.
