...
class InvoiceFilter
{
    ...
    public function apply(Builder $query): Builder
    {
        return $query
            ...
            ->when($this->request->filled('fee'), fn($q) =>
                $q->whereHas('fee', fn($f) =>
            ...
            ->when($this->request->filled('issued_from'), fn($q) =>
                $q->whereDate('issued_date', '>=', $this->request->issued_from)
            )
            ->when($this->request->filled('issued_to'), fn($q) =>
                $q->whereDate('issued_date', '<=', $this->request->issued_to)
            )
            ->when($this->request->filled('due_from'), fn($q) =>
                $q->whereDate('due_date', '>=', $this->request->due_from)
            )
            ->when($this->request->filled('due_to'), fn($q) =>
