...
class StudentScholarshipFilter
{
    ...
    public function apply(Builder $query): Builder
    {
        return $query
            ...
            ->when($this->request->filled('grant_from'), fn($q) =>
                $q->whereDate('grant_date', '>=', $this->request->grant_from)
            )
            ->when($this->request->filled('grant_to'), fn($q) =>
                $q->whereDate('grant_date', '<=', $this->request->grant_to)
            )
            ->when($this->request->filled('paid_from'), fn($q) =>
                $q->whereDate('paid_at', '>=', $this->request->paid_from)
            )
            ->when($this->request->filled('paid_to'), fn($q) =>
                $q->whereDate('paid_at', '<=', $this->request->paid_to)
            )
            ->when($this->request->filled('reference'), fn($q) =>
